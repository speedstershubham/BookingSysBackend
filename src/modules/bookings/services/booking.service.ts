...
import {
  ...
  insertBooking,
  updateBookingSeats,
} from '@/modules/bookings/repository/booking.repository';
import type {
  BookingDetailRow,
  BookingRecord,
  CancelBookingResult,
...
import { lockSeats } from '@/modules/seats/repository/seat.repository';
import {
  getShowtimeSeats,
  isSeatSelectable,
...
export async function createBooking(
  ...
  return saveBooking(userId, input.showtimeId, input.seatIds);
}
...
export async function getBookingById(
  ...
  const rows = await findBookingById(bookingId, userId);
...
export async function cancelBooking(
  ...
  try {
    return await cancelBookingById(bookingId, userId);
  } catch (error) {
...
export async function updateBooking(
  ...
  const existing = await findBookingById(bookingId, userId);
  ...
  const showtime = await getShowtimeSeats(showtimeId, userId);

  const unavailableSeats = showtime.seats.filter(
    (seat) =>
      uniqueSeatIds.includes(seat.id) &&
      !isSeatSelectable(seat, currentSeatIds),
  );
  ...
  try {
    ...
    await updateBookingSeats(userId, bookingId, uniqueSeatIds);

    const updated = await findBookingById(bookingId, userId);
...
async function saveBooking(
  ...
  const unavailableSeats = showtime.seats.filter(
    (seat) => uniqueSeatIds.includes(seat.id) && !isSeatSelectable(seat),
  );
  ...
  try {
    const bookingId = await insertBooking(userId, showtimeId, uniqueSeatIds);
...
function assertShowtimeNotStarted(startTime: Date): void {
  if (startTime.getTime() <= Date.now()) {
    throw new AppError(400, 'Showtime has already started');
  }
}

function handleBookingMutationError(error: unknown): never {
...
function mapBookingRows(rows: BookingDetailRow[]): BookingRecord {
  const first = rows[0];
...
function groupBookingRows(rows: BookingDetailRow[]): BookingRecord[] {
